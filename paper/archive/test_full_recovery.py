...
class Objective:
    """Picklable objective for multiprocessing."""
    ...
    def __call__(self, x):
        try:
            p = self.build_fn(x)
            return float(np.mean((fast_forward(p, 200, 10.0) - self.target)**2))
        except:
            return 1e6


N_WORKERS = 4  # gentle on thermals
...
def run_config(label, build_params_fn, bounds, true_vec, n_restarts=6,
    ...
    obj = Objective(build_params_fn, TARGET)
    ...
    if best_x is not None:
        ...
        for perm in permutations(range(3)):
            ...
            if len(extra) >= 3:
                ...
                rp = minimize(obj, x_perm_g, method='Nelder-Mead',
    ...
    return best_x, best_mse, errors, 0, wt
