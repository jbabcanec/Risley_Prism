...
All optimisation results use the same forward model for both data
generation and fitting.  To quantify the effect of geometry errors, we
generated patterns with perturbed geometry and solved assuming the
default.  A 3\,\% thickness error ($3.0\!\to\!3.1$) yields
$|\Delta N|\!=\!10^{-5}\;\mathrm{Hz}$ (still usable); a 17\,\% error
($3.0\!\to\!3.5$) causes complete failure.  A 5\,\% workpiece-distance
error ($100\!\to\!105$) yields $|\Delta N|\!=\!3\!\times\!10^{-4}\;\mathrm{Hz}$
and $|\Delta\alpha|\!\approx\!0.5^\circ$; a 20\,\% error breaks the
solver.  In practice, geometry is known to $<1\;\%$ from hardware
design, placing the solver well within the safe regime.
