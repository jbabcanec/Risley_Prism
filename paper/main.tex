...
\begin{table}[htbp]
\centering
\caption{Recovery under observation noise ($P=3$).  Errors are
maximum across all prisms.}
\label{tab:noise}
\begin{tabular}{@{}rccc@{}}
\toprule
SNR & $|\Delta N|$ & $|\Delta\alpha_x|$ & $|\Delta\alpha_y|$ \\
(dB) & (Hz)         & (deg)              & (deg) \\
\midrule
$\infty$ & $10^{-15}$ & $10^{-13}$ & $10^{-11}$ \\
60       & $10^{-5}$  & $10^{-3}$  & $10^{-2}$  \\
40       & $10^{-4}$  & $10^{-2}$  & $10^{-1}$  \\
30       & $>1$       & $>10$      & $>10$ \\
\bottomrule
...
Table~\ref{tab:noise} shows graceful degradation down to 40\,dB SNR:
errors scale as $\sim\!\sigma/\sqrt{H_{jj}}$, consistent with the
Hessian prediction~\eqref{eq:hessian}.  At 30\,dB
($\sigma \approx 0.7$ length units) the noise distorts the pattern
beyond recognition: the global optimiser converges to a local minimum
far from the true parameters, producing speed errors exceeding
$1\;\mathrm{Hz}$---the solver returns a plausible but incorrect
configuration.  The practical reliability threshold is
$\sim\!40\;\mathrm{dB}$.
