...
\begin{table}[t]
\centering
\caption{Parameter recovery under observation noise for a 3-prism system.}
\label{tab:noise}
\begin{tabular}{@{}rcccc@{}}
\toprule
SNR (dB) & $\sigma$ & $\max|\Delta N|$ (Hz) & $\max|\Delta\alpha_x|$ (deg)
  & $\max|\Delta\alpha_y|$ (deg) \\
\midrule
$\infty$ & 0       & $8\!\times\!10^{-15}$ & $9\!\times\!10^{-13}$ & $2\!\times\!10^{-11}$ \\
60       & 0.021   & $2\!\times\!10^{-5}$  & $1\!\times\!10^{-3}$  & $6\!\times\!10^{-2}$  \\
40       & 0.21    & $2\!\times\!10^{-4}$  & $1\!\times\!10^{-2}$  & $6\!\times\!10^{-1}$  \\
30       & 0.68    & \multicolumn{3}{c}{solver finds wrong basin} \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:noise} shows that the solver degrades gracefully down to
40\,dB SNR: speed errors scale as $\sim\!\sigma/\sqrt{H_N}$, consistent
with the Hessian prediction~\eqref{eq:hessian}.  At 30\,dB the noise
overwhelms the pattern structure and the solver converges to a spurious
local minimum.  The practical noise floor for reliable recovery is
therefore $\sim\!40\;\mathrm{dB}$ ($\sigma \approx 0.2$ length units).

\subsection{Scaling to $P=4$}

We test the solver on a 4-prism (8-interface) system with 12 free
parameters ($N=[1.5, -1.0, 2.0, -0.7]$,
$\alpha_x=[10, -8, 5, -12]^\circ$, $\alpha_y=[3, 7, -4, 6]^\circ$).
With 5 restarts and 300 DE generations, the solver recovers all
12~parameters to machine precision (MSE~$=2.1\times10^{-23}$) in
621\,k evaluations ($\sim\!17\;\mathrm{min}$).  The maximum errors are
$|\Delta N|<1.3\times10^{-14}\;\mathrm{Hz}$,
$|\Delta\alpha_x|<2.4\times10^{-12}\;\mathrm{deg}$,
$|\Delta\alpha_y|<2.8\times10^{-11}\;\mathrm{deg}$.
...
\begin{table}[t]
\centering
\caption{Yang paraxial FFT vs.\ our method for $P=2$ test cases.
The FFT method can only estimate speeds (not angles); our method
recovers all parameters.}
\label{tab:yang}
\begin{tabular}{@{}lccc@{}}
\toprule
Case & FFT $|\Delta N|$ (Hz) & Ours $|\Delta N|$ (Hz) & Ours $|\Delta\alpha_x|$ (deg) \\
\midrule
Small angles ($3^\circ$)   & 0.50 & $<10^{-15}$ & $<10^{-14}$ \\
Medium angles ($8^\circ$)  & 0.50 & $<10^{-15}$ & $<10^{-13}$ \\
Large angles ($15^\circ$)  & 0.50 & $<10^{-15}$ & $<10^{-14}$ \\
Close speeds (1.0, 1.1)    & 0.90 & $<10^{-15}$ & $<10^{-13}$ \\
\bottomrule
\end{tabular}
\end{table}
...
The NN serves two functions.  First, it classifies the prism count with
99.1\,\% accuracy.  Second, it provides a warm start for the global
optimiser.  To quantify the warm-start benefit, we ran an ablation
comparing NN+DE (first restart seeded with the NN estimate) versus
DE-only (all restarts random).  Both achieve the same final MSE
($\sim\!10^{-24}$), but NN+DE is $1.4\times$ faster for 3-prism cases
(75\,s vs.\ 107\,s) because the NN-seeded restart reaches the global
basin earlier.  For 2-prism cases, the benefit is negligible (the
search space is small enough that random restarts suffice).  The NN's
contribution is therefore modest but real---a $\sim\!30\;\mathrm{s}$
saving per 3-prism solve.
...
We have demonstrated $P=4$ recovery (12-D search) in
Sec.~\ref{sec:results}: all parameters recovered to machine precision
in $\sim\!17\;\mathrm{min}$ (621k evaluations).  The forward-model cost
...
generation and fitting.  When the assumed geometry differs from the true
geometry (e.g., incorrect prism thickness or workpiece distance), the
solver converges to a biased solution with non-zero residual MSE.  We
quantify this effect in a model-mismatch study: a 3\,\% thickness error
produces speed errors of $\sim\!10^{-3}\;\mathrm{Hz}$, while a 20\,\%
workpiece-distance error produces $\sim\!0.1\;\mathrm{Hz}$ speed errors.
In practice, geometry is usually known to $<1\;\%$ from hardware design.
...
\item \textbf{Quantified noise robustness}: graceful degradation down
      ...
      $\sigma/\sqrt{H_N}$.
\item \textbf{NN ablation}: the neural-network warm start provides a
      measured $1.4\times$ speedup over DE-only for $P=3$.
