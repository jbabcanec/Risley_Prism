...
all parameters are recovered to machine precision
(MSE~$\sim\!10^{-23}$).  A 30-trial-per-SNR noise study shows graceful
degradation (100\,\% success at 60\,dB, 57\,\% at 35\,dB).
Model-mismatch tests quantify tolerance to geometry errors ($<\!3$\,\%
thickness error is safe).  An NN ablation reveals that the network's
value is classification, not regression warm-starting.  The solver
outperforms the paraxial FFT inverse by $>$14 orders of magnitude.
\end{abstract}
...
\item \textbf{Machine-precision recovery} on 23 test cases ($P=2$--$4$)
...
optimisation recovers all parameters to machine precision.
...
Sec.~\ref{sec:results}: all parameters recovered to machine precision
...
The noise robustness study (Table~\ref{tab:noise}) shows reliable
recovery down to 40\,dB SNR, with a sharp transition to failure between
35 and 30\,dB.  The multi-trial study (Sec.~\ref{sec:noise})
quantifies the variance across noise realisations.  Real scan patterns
will additionally include beam jitter and timing uncertainty, which may
have different statistical structure than the additive Gaussian model
tested here.
...
\item \textbf{Machine-precision recovery} on 19 test cases including
      adversarial configurations (near-identical speeds, tiny angles,
      harmonic ratios), up to $P=4$ (12-D, 8 interfaces).
...
and (iii)~experimental validation on physical Risley prism hardware.
