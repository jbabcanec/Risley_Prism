...
class StateOfTheArtSolver:
    ...
    def forward_simulate(self, params):
        """Generate pattern using physics bridge (real or fallback)."""

        # Try to use real physics bridge
        try:
            from core.physics_bridge import RisleyPhysicsEngine
            ...
            pattern = engine.simulate_real_pattern(
                ...
                phi_x=params['phi_x'],
                phi_y=params['phi_y'],
                time_points=60,  # Match existing pattern length
                time_duration=2.0,
                distances=params.get('distances'),
                refractive_indices=params.get('refractive_indices')
            )
            ...
            # Fall back to improved physics-inspired simulation
            pass

        # Improved fallback pattern generation (better than pure sine waves)
        wedge_count = params['wedgenum']
        rotation_speeds = params['rotation_speeds']
        phi_x = params['phi_x']
        phi_y = params['phi_y']
        ...
        # Each wedge contributes based on rotation and angles
        for i in range(wedge_count):
            if i < len(rotation_speeds) and i < len(phi_x) and i < len(phi_y):
                # Rotation frequency
                freq = rotation_speeds[i]

                # Deflection amplitudes proportional to wedge angles
                # Real physics: deflection ≈ (n-1) * wedge_angle
                n = 1.5  # Typical glass refractive index
                deflection_x = (n - 1) * phi_x[i] * np.pi / 180.0  # Convert to radians
                deflection_y = (n - 1) * phi_y[i] * np.pi / 180.0

                # Phase based on rotation
                phase = 2 * np.pi * freq * t

                # Rotating deflection vector
                x_pattern += deflection_x * np.cos(phase) - deflection_y * np.sin(phase)
                y_pattern += deflection_x * np.sin(phase) + deflection_y * np.cos(phase)

        # Scale to reasonable range (millimeters at workpiece)
        distance_to_workpiece = 100.0  # mm
        x_pattern *= distance_to_workpiece
        y_pattern *= distance_to_workpiece

        # Add small noise for realism
        noise_level = 0.01
        x_pattern += np.random.normal(0, noise_level, len(t))
        y_pattern += np.random.normal(0, noise_level, len(t))
