...
class PatternDataset(Dataset):
    ...
    def _normalize_pattern(self, pattern: np.ndarray) -> np.ndarray:
        ...
        # Normalize coordinates using min-max scaling (preserves shape)
        # Find the bounding box
        x_min, x_max = pattern[:, 0].min(), pattern[:, 0].max()
        y_min, y_max = pattern[:, 1].min(), pattern[:, 1].max()

        # Scale to [0, 1] range while preserving aspect ratio
        x_range = x_max - x_min
        y_range = y_max - y_min
        max_range = max(x_range, y_range) + 1e-6  # Avoid division by zero

        # Normalize to unit square preserving shape
        pattern[:, 0] = (pattern[:, 0] - x_min) / max_range
        pattern[:, 1] = (pattern[:, 1] - y_min) / max_range
