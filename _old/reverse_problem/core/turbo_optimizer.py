...
class TurboOptimizer:
    ...
    def _cpu_optimize(self, pattern: np.ndarray, wedge_count: int,
        ...
        from .genetic_algorithm import solve_reverse_problem
        ...
        return {
            ...
            'cost': cost,
            'generations_used': info.get('generations', ga_params['generations']),
            'population_size': ga_params['population_size']
        }
    
    def _array_to_param_dict(self, array: np.ndarray, wedge_count: int) -> Dict:
        """Convert parameter array to dictionary format."""
        params = {
            'wedgenum': wedge_count,
            'rotation_speeds': array[:wedge_count].tolist() if wedge_count > 0 else [],
            'phi_x': array[wedge_count:2*wedge_count].tolist() if wedge_count > 0 else [],
            'phi_y': array[2*wedge_count:3*wedge_count].tolist() if wedge_count > 0 else [],
